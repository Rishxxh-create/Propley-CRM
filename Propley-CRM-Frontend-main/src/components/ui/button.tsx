  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-xs font-semibold whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-2 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        ...
        propley: "bg-gradient-to-r from-ink via-zinc-800 to-ink bg-[length:200%_auto] text-white py-6 text-xs font-semibold transition-all duration-500 hover:from-gold hover:via-[#b3966a] hover:to-gold hover:bg-[position:100%_0%] select-none flex items-center justify-center gap-3 rounded-md",
      },
      size: {
        default:
...
function Button({
  ...
  return (
    ...
    >
      {loading && (
        <RiLoader4Line className="animate-spin text-current shrink-0 mr-2" size={16} />
      )}
      {children}
    </ButtonPrimitive>
  )
