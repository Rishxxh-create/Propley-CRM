...
import { fetchMeetingMetadata, fetchEnablexToken } from '@/lib/api/meetings';
...
export function useParticipantSession(
  ...
  const [meeting, setMeeting] = useState<ApiMeetingMetadata | null>(null);
  ...
  const [unmuteRequestSocketId, setUnmuteRequestSocketId] = useState<string | null>(null);
  ...
  const stopLocalMedia = useCallback(() => {
    try {
      ...
      if (!local) return;
      if (typeof local.close === 'function') {
        local.close();
        return;
      }
      if (typeof local.getTracks === 'function') {
  ...
  // 1. Fetch meeting metadata immediately on mount
  useEffect(() => {
    let active = true;

    async function getMetadata() {
      try {
        const data = await fetchMeetingMetadata(roomId);
        if (!active) return;
        if (data) {
          setMeeting(data);
          setPhase('authorized');
          ...
          setPhase('denied');
          toast.error("Meeting session not found.");
        }
      } catch (err: any) {
        if (!active) return;
        setPhase('denied');
  ...
  // Bootstrap slide 1 locally until moderator urlChange (or recovered payload) arrives.
  useEffect(() => {
    if (phase !== 'authorized' || currentUrl) return;
    let cancelled = false;
    fetchMandakeSlides()
      .then((slides) => {
        if (cancelled || slides.length === 0) return;
        const defaultId = getDefaultSlideId(slides);
        const slide = defaultId ? getSlideById(slides, defaultId) : slides[0];
        if (!slide) return;
        setCurrentUrl((prev) => prev ?? getMandakeSlideSrc(slide.path));
      })
      .catch(() => {
        /* wait for socket urlChange */
      });
    return () => {
      cancelled = true;
    };
  ...
  // 2. Setup Socket connection & join room as participant
  useEffect(() => {
    if (phase !== 'authorized' || !userName) return;
    ...
    registerSocketDebugger(socket, 'participant');
    ...
    socket.on('connect', async () => {
      ...
      try {
        ...
        if (!active) return;
        console.error('Failed to fetch EnableX token for participant:', err);
    ...
    socket.on('urlChange', (payload: any) => {
      if (!active) return;
      console.log('urlChange received in participant:', payload);
      if (payload?.src) {
        setCurrentUrl(payload.src);
      }
    });
    ...
    socket.on('session-ended', () => {
      ...
      // Ensure EnableX fully disconnects and devices are released.
      try {
        if (roomRef.current?.room?.disconnect) {
          roomRef.current.room.disconnect();
        }
      } catch { /* ignore */ }
      stopLocalMedia();
    ...
    socket.on('force-mute-self', (payload: any) => {
      if (!active) return;
      console.log('force-mute-self received by participant:', payload);
      toast.success(`You have been muted by ${payload.by || 'Advisor'}`);
      // Moderator forced a mute — sync intent + icon to the now-muted SDK state.
      desiredAudioOnRef.current = false;
      if (roomRef.current && roomRef.current.localStream) {
        const local = roomRef.current.localStream;
        if (local.muteAudio) {
          local.muteAudio(() => {
            setAudioEnabled(false);
            if (socketRef.current?.connected) {
              socketRef.current.emit('mic-state-changed', { isMuted: true, by: '' });
            }
          });
        }
      }
    ...
    socket.on('unmute-request', (payload: { by: string; from: string }) => {
      if (!active) return;
      console.log('unmute-request received by participant:', payload);
      setUnmuteRequestFrom(payload.by);
      setUnmuteRequestSocketId(payload.from);
    });
  ...
  }, [phase, roomId, userName, phone, router, onEnd]);
    ...
    (window as any).EnxRtc.joinRoom(enablexToken, streamInfo, (room: any, err: any) => {
      if (!isMounted) {
        if (room && room.room && room.room.disconnect) {
          try { room.room.disconnect(); } catch (e) {}
        }
        return;
      }
      ...
      if (err) {
        ...
        if (isStaleRoom && joinRetryRef.current < 1 && socketRef.current?.id) {
          // Re-fetch a fresh token — /token self-heals a deleted room — and retry once.
          joinRetryRef.current += 1;
          fetchEnablexToken({ uuid: roomId, name: userName, role: 'participant', socketId: socketRef.current.id })
      ...
      if (room) {
        console.log('Participant successfully joined EnableX room:', room);
        ...
        if (localStream) {
          ...
          // EnxRtc.startScreenShare reaches into a global `localStream` — expose ours.
        ...
        // Capture the active input device ids (for the device picker) and load the device list.
        try {
        ...
        const surfaceShare = (rm: any) => {
          ...
          if (!shared) return;
          shared.play(SHARE_CONTAINER_ID.participant, ENX_SHARE_PLAYER_OPTIONS);
          setScreenShareStream(shared);
          setScreenSharePresenter(remoteSharerRef.current?.name || 'Presenter');
          playSharedScreen(SHARE_CONTAINER_ID.participant);
        };
        ...
        if (actualRoom) {
          ...
          actualRoom.addEventListener('share-started', (event: any) => {
            ...
            remoteSharerRef.current = { clientId: sharerClientId, name: event?.message?.name || 'Presenter' };
          ...
          actualRoom.addEventListener('share-stopped', () => {
            ...
            const node = document.getElementById(SHARE_CONTAINER_ID.participant);
          ...
          if (actualRoom.shareStatus && !actualRoom.isSharingClient && joinInfo?.clientId && joinInfo.clientId !== actualRoom.clientId) {
            remoteSharerRef.current = { clientId: joinInfo.clientId, name: joinInfo.name || 'Presenter' };
            const shared = actualRoom.remoteStreams?.get?.(SCREEN_SHARE_STREAM_ID);
            if (shared) {
              if (shared.stream) surfaceShare(actualRoom);
              else { try { actualRoom.subscribe(shared); } catch { /* stream-subscribed handles play */ } }
  ...
  }, [enablexToken, userName, stopLocalMedia, refreshDevices, roomId]);
  ...
  const toggleScreenShare = useCallback(() => {
    ...
    try {
      ...
      if (share) {
        ...
        cropShareToElement(() => room.ScreenSharelocalStream ?? share, PRESENTATION_STAGE_ID.participant);
        // The browser's "Stop sharing" control ends the track — mirror that into EnableX.
  ...
  const acceptUnmuteRequest = useCallback(() => {
    if (!unmuteRequestSocketId) return;
    setUnmuteRequestFrom(null);
    setUnmuteRequestSocketId(null);
    toggleLocalAudio(true);
  }, [unmuteRequestSocketId, toggleLocalAudio]);

  const declineUnmuteRequest = useCallback(() => {
    if (!unmuteRequestSocketId) return;
    if (socketRef.current?.connected) {
      socketRef.current.emit('participant:unmute-declined', { to: unmuteRequestSocketId });
    }
    setUnmuteRequestFrom(null);
    setUnmuteRequestSocketId(null);
  }, [unmuteRequestSocketId]);
